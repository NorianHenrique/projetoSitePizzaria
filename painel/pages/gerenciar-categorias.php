<?php
	if(isset($_GET['excluir'])){
		...
		$noticias = MySql::conectar()->prepare("SELECT * FROM `tb_admin.pizzas` WHERE categoria_id = ?");
		...
		$noticias = MySql::conectar()->prepare("DELETE FROM `tb_admin.pizzas` WHERE categoria_id = ?");
		$noticias->execute(array($idExcluir));
		Painel::redirect(INCLUDE_PATH_PAINEL.'gerenciar-categorias');
	}else if(isset($_GET['order']) && isset($_GET['id'])){
		Painel::orderItem('tb_site.categoria',$_GET['order'],$_GET['id']);
	}

	$paginaAtual = isset($_GET['pagina']) ? (int)$_GET['pagina'] : 1;
	$porPagina = 4;
	
	$categorias = Painel::selectAll('tb_site.categoria',($paginaAtual - 1) * $porPagina,$porPagina);
...
<div class="box-content">
	...
	<div class="paginacao">
		<?php
			$totalPaginas = ceil(count(Painel::selectAll('tb_site.categoria')) / $porPagina);
